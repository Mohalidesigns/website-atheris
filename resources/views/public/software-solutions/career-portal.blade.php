                ...
                <div class="hidden lg:block">
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                        @if($product && $product->hero_image)
                            <img src="{{ asset('storage/' . $product->hero_image) }}" alt="Career Portal Homepage" class="rounded-xl w-full aspect-[4/3] object-cover object-top">
                        @elseif(file_exists(public_path('images/products/career-portal-home.png')))
    ...
    <section class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                @foreach([
                    ['field' => 'screenshot_1', 'caption_field' => 'screenshot_1_caption', 'alt' => 'Job Listings', 'fallback_img' => 'career-portal-jobs.png', 'default_caption' => 'Browse open positions with search, filters, and salary transparency'],
                    ['field' => 'screenshot_2', 'caption_field' => 'screenshot_2_caption', 'alt' => 'Job Detail Page', 'fallback_img' => 'career-portal-detail.png', 'default_caption' => 'Detailed job pages with requirements, benefits, and one-click apply'],
                    ['field' => 'screenshot_3', 'caption_field' => 'screenshot_3_caption', 'alt' => 'Career Portal Homepage', 'fallback_img' => 'career-portal-home.png', 'default_caption' => 'Branded homepage with featured positions and category browsing'],
                ] as $ss)
                <div class="bg-bg rounded-2xl p-4 border border-border">
                    @if($product && $product->{$ss['field']})
                        <img src="{{ asset('storage/' . $product->{$ss['field']}) }}" alt="{{ $product->{$ss['caption_field']} ?? $ss['alt'] }}" class="rounded-xl w-full object-cover object-top border border-border">
                    @elseif(file_exists(public_path('images/products/' . $ss['fallback_img'])))
                        <img src="{{ asset('images/products/' . $ss['fallback_img']) }}" alt="{{ $ss['alt'] }}" class="rounded-xl w-full object-cover object-top border border-border">
                    @else
                        <div class="bg-white rounded-xl aspect-[4/3] flex items-center justify-center text-text-secondary/30 border border-dashed border-border">
                            <div class="text-center">
                                ...
                                <p class="text-xs font-medium">{{ $ss['alt'] }}</p>
                            </div>
                        </div>
                    @endif
                    <p class="text-sm text-text-secondary mt-3 text-center font-medium">{{ ($product ? $product->{$ss['caption_field']} : null) ?? $ss['default_caption'] }}</p>
                </div>
                @endforeach
            </div>
        </div>
